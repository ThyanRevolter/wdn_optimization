...
def mpc_wrapper(request):
    ...
    return MPCWrapper(mpc_params)


@pytest.mark.parametrize("mpc_wrapper, expected", MPC_TEST_CASES, indirect=["mpc_wrapper"])
def test_mpc_time_horizons(mpc_wrapper, expected):
    """Test that the MPC time horizons are created correctly."""
    assert mpc_wrapper.mpc_time_horizon == expected["expected_horizons"]

@pytest.mark.parametrize("mpc_wrapper, expected", MPC_TEST_CASES, indirect=["mpc_wrapper"])
def test_mpc_run_shape_and_columns(mpc_wrapper, expected):
    ...
    for i, result_df in results.items():
        ...
        assert result_df["Datetime"].iloc[-1] == expected["expected_horizons"][i]["optimization_end_time"] - timedelta(hours=mpc_wrapper.simulation_time_step/3600)

@pytest.mark.parametrize("mpc_wrapper, expected", MPC_TEST_CASES, indirect=["mpc_wrapper"])
def test_mpc_tank_level_continuity(mpc_wrapper, expected):
    ...
    temp_params = mpc_wrapper.params.copy()
    temp_params["optimization_start_time"] = mpc_wrapper.simulation_start_date.strftime("%Y-%m-%d %H:%M:%S")
    temp_params["optimization_end_time"] = mpc_wrapper.simulation_end_date.strftime("%Y-%m-%d %H:%M:%S")
    temp_params["optimization_time_step"] = mpc_wrapper.simulation_time_step
    ...
    for tank_name in tank_names:
        ...
        for horizon_idx in range(1, len(results)):
            current_horizon = results[horizon_idx]
            previous_horizon = results[horizon_idx - 1]
            
            # Get the start time of the current horizon
            current_start_time = mpc_wrapper.mpc_time_horizon[horizon_idx]["optimization_start_time"]
            ...
            previous_horizon_start = mpc_wrapper.mpc_time_horizon[horizon_idx - 1]["optimization_start_time"]
            time_diff = (current_start_time - previous_horizon_start).total_seconds()
            time_step_seconds = mpc_wrapper.simulation_time_step
